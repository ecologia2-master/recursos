...
author: "Basada en Smith & Smith y Odum–Barrett <br> Arlen Marmolejo"
...
| **Símbolo**                         | **Significado**                                                                    |
|-------------------------------------|------------------------------------------------------------------------------------|
| $$N_1,\ N_2$$                       | Tamaños poblacionales de las especies 1 y 2 (individuos).                         |
| $$r_1,\ r_2$$                       | Tasas de crecimiento intrínsecas (tiempo\(^{-1}\)).                               |
| $$K_1,\ K_2$$                       | Capacidades de carga para las especies 1 y 2 (individuos).                        |
| $$\alpha_{12}$$                     | Efecto per cápita de la especie 2 sobre la 1 (adimensional, en “unidades” de 1).  |
| $$\alpha_{21}$$                     | Efecto per cápita de la especie 1 sobre la 2 (adimensional, en “unidades” de 2).  |
| $$t$$                               | Tiempo.                                                                            |
| $$\dfrac{dN_1}{dt},\ \dfrac{dN_2}{dt}$$ | Tasas instantáneas de cambio poblacional (individuos·tiempo\(^{-1}\)).       |
| $$N_1^{*},\ N_2^{*}$$               | Abundancias de equilibrio (si existe coexistencia).                                |
]
...
- $N(t)$ presas; $P(t)$ depredadores;
- $t$ tiempo;
- $\frac{dN}{dt},\,\frac{dP}{dt}$ tasas de cambio;
- $r$ crecimiento intrínseco (presas);
- $a$ tasa de encuentro/ataque;
- $b$ eficiencia de conversión;
- $m$ mortalidad (depredadores).
...
**Simbología:**
- $p(t)$ fracción de parches ocupados (0–1);
- $c$ tasa de colonización por parche ocupado (tiempo$^{-1}$);
- $e$ tasa de extinción local (tiempo$^{-1}$);
- $\frac{dp}{dt}$ cambio instantáneo en la fracción ocupada;
- $p^*$ ocupación en equilibrio (si $c>e$).

---

```{r}
levins <- function(p0=0.3, c=0.4, e=0.2, tmax=150){
  p <- numeric(tmax); p[1] <- p0
  for (t in 2:tmax) {
    p[t] <- p[t-1] + c*p[t-1]*(1 - p[t-1]) - e*p[t-1]
    p[t] <- max(0, min(1, p[t]))  # opcional: mantener p en [0,1]
  }
  list(t = 1:tmax, p = p, pstar = 1 - e/c)
}

res <- levins(tmax = 150)
plot(res$t, res$p, type='l', lwd=2, ylim=c(0,1),
     xlab='t', ylab='p (parches ocupados)')
abline(h = res$pstar, lty=2, col=2)
```
